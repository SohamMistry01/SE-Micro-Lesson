...
from modules.microlesson_generator.orchestrator import process_microlessons_logic
from modules.microlesson_generator.prompt_config import DEFAULT_CUSTOM_PROMPT

router = APIRouter()

class MicroLessonRequest(BaseModel):
    category: str = Field(default="general", description="Category of the micro-lesson (e.g., 'theoretical', 'practical', 'general')")
    template_path: str = Field(
        default="default", 
        description="Key for the template to use (e.g., 'default'). Maps to predefined paths in the backend."
    )
    priority_llm: Optional[str] = Field(default=None, description="Optional LLM model name to prioritize for this task.")
    ...
    custom_prompt: Optional[str] = Field(
        default=None, 
        example=DEFAULT_CUSTOM_PROMPT, # Populates Swagger UI without forcing a default
