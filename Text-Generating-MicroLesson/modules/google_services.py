...
def fetch_input_data():
    ...
    result = service.spreadsheets().values().get(
        spreadsheetId=settings.SPREADSHEET_ID, range=range_name
    ).execute()
...
def get_and_parse_drive_file(file_id: str) -> str:
    ...
    try:
        ...
        # 2. ✅ FIX: Handle PDFs using Google's Native Conversion Engine
            ...
            temp_doc = drive_service.files().copy(
                fileId=file_id, 
                body=copy_metadata,
                supportsAllDrives=True
            ).execute()
            ...
            try:
                ...
                request = drive_service.files().export_media(fileId=temp_doc_id, mimeType='text/plain')
                fh = io.BytesIO()
                downloader = MediaIoBaseDownload(fh, request)
                done = False
                while not done:
                ...
                try:
                    drive_service.files().delete(fileId=temp_doc_id, supportsAllDrives=True).execute()
                except Exception as cleanup_err:
                    print(f"   Warning: Could not delete temp file {temp_doc_id}: {cleanup_err}")
