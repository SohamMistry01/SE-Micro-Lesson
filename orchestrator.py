...
import io
import re
import PyPDF2 # <- pip install pypdf==3.0.1
from typing import Optional
from datetime import datetime
from collections import defaultdict
from modules.microlesson_generator.config import settings
from modules.microlesson_generator import google_services as gs
from modules.microlesson_generator import rag_engine
from modules.microlesson_generator import pdf_generator
from modules.microlesson_generator.prompt_config import get_resolved_prompt
...
def process_microlessons_logic(
    ...
    final_template_path = TEMPLATE_MAP.get(template_path.lower(), settings.DEFAULT_TEMPLATE_PATH)
    ...
    for i, row in enumerate(data_rows):
        ...
        if status.strip().lower() == 'no':
            tasks[uid].append({"row_index": sheet_row_index, "content_data": content_cell})
    ...
    for uid, items in tasks.items():
        ...
        for item in items:
            ...
            if text and text.strip():
                ...
                print(f"   ⚠️ Warning: Extracted text is completely empty. If this is a PDF, it may be a scanned image with no readable text layer.")
        ...
        txt_link = gs.upload_to_drive(txt_filename, lesson_md.encode('utf-8'), 'text/plain')
